...
class Rcon {
  static send(host, port, pass, cmd) {
    return new Promise((resolve, reject) => {
      ...
      socket.on("close", () => resolve(responseText.trim()));
    });
  }
}
...
async function run() {
  // Announce in chat
  await Rcon.send(host, port, pass, 'say "Antigravity: Loading match configuration via exact RCON command..."');
  
  // Load configuration with exact quotes
  const cmd = 'matchzy_loadmatch_url "https://esports-bracket-flow--buffinger1.replit.app/api/matchzy/active-match.json"';
  console.log(`📡 Sending RCON command: ${cmd}`);
  const res = await Rcon.send(host, port, pass, cmd);
